...
public class NewsSubscribe {
	...
	@RequestMapping(value = "newsSubscribe", method = RequestMethod.POST)
	public ModelAndView event(HttpServletRequest request, @RequestParam String news) {
		...
		if (news.equalsIgnoreCase("Unsubscribed")) {
			news = "Subscribed";
			user.setNagarroNews(true);

		} else if (news.equalsIgnoreCase("subscribed")) {
			news = "Unsubscribed";
			user.setNagarroNews(false);
		}
